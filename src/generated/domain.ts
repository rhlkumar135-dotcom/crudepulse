...
export class CachedDataStore {
  items: Map<string, CachedDataType> = new Map()
  isLoading = false
  error: string | null = null
  ...
  /** Get all items as array */
  get all(): CachedDataType[] {
    return Array.from(this.items.values())
  }

  /** Get item by ID */
  get(id: string): CachedDataType | undefined {
    return this.items.get(id)
  }
  ...
  async loadAll(userId?: string, where?: Record<string, unknown>) {
    ...
    try {
      const items = await getCachedDataList({ data: { userId, where } })
  ...
  async loadById(id: string, userId?: string) {
    try {
      const item = await getCachedDataById({ data: { id, userId } })
  ...
  /** Create new item with optimistic update */
  async create(input: CachedDataCreateInput, userId?: string) {
    // Create optimistic item
    const tempId = `temp-${crypto.randomUUID()}`
    const optimisticItem: CachedDataType = {
      id: tempId,
      ...input,
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    try {
      const item = await createCachedData({ data: { input, userId } })
  ...
  /** Update item with optimistic update */
  async update(id: string, input: CachedDataUpdateInput, userId?: string) {
    // Validate id to prevent undefined from reaching the API
    if (!id || typeof id !== 'string') {
      console.error('[CachedDataStore] update called with invalid id:', id)
      return
    }
    const existing = this.items.get(id)
    if (!existing || this.pendingUpdates.has(id)) return
    ...
    try {
      const item = await updateCachedData({ data: { id, input, userId } })
  ...
  async delete(id: string, userId?: string) {
    // Validate id to prevent undefined from reaching the API
    if (!id || typeof id !== 'string') {
      console.error('[CachedDataStore] delete called with invalid id:', id)
      return
    }
    const existing = this.items.get(id)
    if (!existing || this.pendingDeletes.has(id)) return
    ...
    try {
      await deleteCachedData({ data: { id, userId } })
...
export class RootStore {
  user: UserStore
  cachedData: CachedDataStore

  constructor() {
    this.user = new UserStore()
    this.cachedData = new CachedDataStore()
    makeAutoObservable(this)
  }
