...
/**
 * Auto-generated CachedData Routes
 *
...
import type { CachedDataHooks } from "./cached-data.hooks"
...
/**
 * Set hooks for CachedData routes
 */
export function setCachedDataHooks(h: CachedDataHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["key", "source", "data", "expiresAt", "createdAt"] as const
...
/**
 * Create CachedData CRUD routes
 */
export function createCachedDataRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.cachedData.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[CachedData] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.cachedData.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CachedData not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CachedData] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.cachedData.create({
        data: picked.data,
      })
      ...
      console.error("[CachedData] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.cachedData.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[CachedData] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[CachedData] Delete error:", error)
